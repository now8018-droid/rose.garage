...
createApp({
  components: { Sidebar, VehicleCard, RenameModal },
  setup() {
    ...
    const normalizeVehicles = (payload) => {
      ...
      return list.map((item) => {
        ...
        const status = stored ? 'garage' : 'pound';
        const imageKey = item.img || 'model_car1';
        return {
    ...
    const filteredVehicles = computed(() => {
      ...
      return vehicles.value
        ...
        .filter((vehicle) => {
          if (!q) return true;
          return [vehicle.name, vehicle.plate, vehicle.className].join(' ').toLowerCase().includes(q);
        })
    ...
    const toggleFavorite = (plate) => {
      ...
      localStorage.setItem('fav-list', JSON.stringify([...favoriteSet]));
    ...
    const handleMessage = (event) => {
      ...
      switch (data.action) {
        ...
        case 'closeui':
          ...
          break;
        case 'pounddeposit':
          poundDeposit.value = Boolean(data.pounddeposit);
          break;
        case 'syncData':
